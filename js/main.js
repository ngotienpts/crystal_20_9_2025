document.addEventListener("DOMContentLoaded", function () {
    ...
    var scrollDetailHandbooks = document.querySelectorAll(
     ...
     var title_news = $('.detail-title').text().trim();
     var numimg = 0;
    $('.detail-content figure').each(function() {
        if ($(this).find('figcaption').length === 0) {
            $(this).append('<figcaption>Ảnh minh họa (Nguồn: Internet)</figcaption>');
        }
        var altimg = $(this).find('img').attr('alt');
        altimg = altimg.trim();
        if(!altimg || altimg==''){
            numimg++;
            $(this).find('img').attr('alt',title_news+' - ảnh '+numimg);
        }
    });
